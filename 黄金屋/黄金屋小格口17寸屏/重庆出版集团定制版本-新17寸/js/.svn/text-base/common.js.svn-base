...
function autoOpenCloseLight(now_time) {					
	var start_light_time = localStorage.getItem(global_ajaxObject.setting.method_start_Light);
	var end_light_time = localStorage.getItem(global_ajaxObject.setting.method_end_Light);
...
function CompareDate(t1,t2){ //比较时间大小
    var date = new Date();  
    var a = t1.split(":");  
    var b = t2.split(":");  
