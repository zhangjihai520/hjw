const app = getApp()

var GetReq = require('../../utils/http.js');
var util = require('../../utils/util.js');

Page({

  /**
   * 页面的初始数据
   */
  data: {

  },

  /**
   * 生命周期函数--监听页面加载
   */
  onLoad: function (options) {
    var t = this;
    t.getLibraryByOrgId();
  },

  /**
   * 生命周期函数--监听页面初次渲染完成
   */
  onReady: function () {

  },

  /**
   * 生命周期函数--监听页面显示
   */
  onShow: function () {

  },

  /**
   * 生命周期函数--监听页面隐藏
   */
  onHide: function () {

  },

  /**
   * 生命周期函数--监听页面卸载
   */
  onUnload: function () {

  },

  /**
   * 页面相关事件处理函数--监听用户下拉动作
   */
  onPullDownRefresh: function () {

  },

  /**
   * 页面上拉触底事件的处理函数
   */
  onReachBottom: function () {

  },

  /**
   * 用户点击右上角分享
   */
  onShareAppMessage: function () {

  },

  /**
   * 获取机构名下拥有的图书馆
   */
  getLibraryByOrgId: function () {
    var t = this;
    var data = {
      orgId: 57
    };
    util.MD5Data(data);
    GetReq.GetReq('getLibraryByOrg', data, function (res) {
      console.info(JSON.stringify(res));
      var dockNameMap = [];
      for (var i = 0; i < res.list.length; i++) {
        dockNameMap.push(res.list[i].dockName);
      }
      t.setData({
        libraryArray: res.list,
        dockNameMap: dockNameMap,
        dockOrg: res.list[0].dockOrg
      })
    });
  },

  bindChange: function (e) {
    var t = this;
    t.setData({
      dockName: t.data.libraryArray[e.detail.value].dockName
    });

    for (var i = 0; i < t.data.libraryArray.length; i++) {
      if (t.data.dockName == t.data.libraryArray[i].dockName) {
        t.setData({
          dockOrg: t.data.libraryArray[i].dockOrg
        });
      }
    }
  },

  /**
  * 监听输入卡号
  */
  inputCard: function (event) {
    var t = this;
    t.setData({
      identityN: event.detail.value
    });
  },

  /**
  * 监听输入姓名
  */
  inputName: function (event) {
    var t = this;
    t.setData({
      realname: event.detail.value
    });
  },

  /**
    * 绑定
    */
  bandingBtn: function () {
    var t = this;
    var data = {
      userId: app.globalData.userId,
      dockOrg: t.data.dockOrg,
      realname: t.data.realname,
      identityN: t.data.identityN
    };
    util.MD5Data(data);
    GetReq.GetReq('handle_Card', data, function (res) {
      console.info(JSON.stringify(res));
      if (res.result == 1) {
        wx.showToast({
          title: res.msg,
        })
        wx.navigateBack({

        })
      } else {
        wx.showToast({
          icon: 'none',
          title: res.msg,
        })
      }
    });
  },
})