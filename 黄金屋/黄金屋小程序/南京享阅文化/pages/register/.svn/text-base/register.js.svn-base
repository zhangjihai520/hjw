...
Page({
  ...
  onLoad: function (options) {
    ...
    if (app.globalData.lvtcode != null){
      ...
      app.globalData.lvtcode = getApp().data.lvtcode;
      lvtcode = app.globalData.lvtcode;
    }
  ...
  inputVerificationCode: function (event) {
    ...
    if (event.detail.cursor == 6 && t.data.check_value) {
      t.setData({
  ...
  submitVerificationCode: function () {
    var t = this;
    if (lvtcode.length == 5 && t.data.check_value){
      var data = {
      ...
      if (t.data.VCodeSize == 6 && t.data.check_value) {
        t.setData({
          view2: true,
          view3: false
        });
  ...
  toUser:function(){
    ...
    wx.downloadFile({
      // 示例 url，并非真实存在
      url: 'https://huangjw.kuguanyun.net/upload/jyg/onfile/享阅文化服务用户协议1.0版.doc',
      success: function (res) {
