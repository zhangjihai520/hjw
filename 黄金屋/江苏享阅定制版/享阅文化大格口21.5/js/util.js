...
//				console.log('beforeSend:::' + JSON.stringify(setting));
			...
			_mui.ajax(url, {
				...
				error:function(xhr,type,errorThrown){
					...
					//异常处理；
					mui.toast(global_stringObject.wangluoyichang, { duration:'long', type:'div' });
				}
			});
		},
		...
		getJSONWaiting: function ajax_getJSONWaiting(url, data, success){
			...
			_mui.ajax(url, {
				...
				error:function(xhr,type,errorThrown){
					...
					//异常处理；						
					mui.toast(global_stringObject.wangluoyichang, {duration:'long', type:'div' });								  
					mui.alert(url, "访问地址失败:"+JSON.stringify(data), function() {});
		...
		getJSONWaitingText	: function ajax_getJSONWaiting(url, data, success){
			var _mui = mui,
				plusNativeUI = plus.nativeUI;			
			    plusNativeUI.showWaiting();			
			_mui.ajaxSettings.beforeSend = function(xhr, setting) {
	...
	helper: {
		checkUpdate: function(){		 
		 ...
		 plus.runtime.getProperty(plus.runtime.appid,function(inf){
            	var data = {version: inf.version};
            	util.ajax.getJSON(url, data, function(res){          

                var  newVersionUrl=global_absoluteRoot+res.url;       
		...
		downloadNewVersion: function(url,callback){
			...
			var downLoader = plus.downloader.createDownload(url, {method: 'GET'}, function(download, status) {
				if(status == 200){
					console.log("200： ");
				}else{
					mui.toast("更新下载失败");					
		...
		downloadNewMovie: function(url,movieName) {
		...
		openWindow: function(url,param){
			...
			/*	
			var newWebview = plus.webview.create(url, url);
			newWebview.addEventListener("loaded",function(){	
				newWebview.show("pop-in", 300, function(){
					newWebview = null;		
				});
			...
			*/
			/*
			var nw = null;
			nw = plus.webview.create(url, url);
