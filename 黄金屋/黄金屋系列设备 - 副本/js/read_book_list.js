...
function book_list_getBookTypeList() {
	var url = createAjaxURLForRead(interface_name.method_book_type_list);
	var data = {
...
function ListMain(direction) { //图书主体
	var url = createAjaxURLForRead(interface_name.method_lbr_ebooks_list);
	...
	var success = function(response) {
		...
		for(var i = 0; i < DataMain.length; i++) {
			...
			var MenuHtml = '<li class="book_item tappable" data-action="book_item" data-bookpic="' + Item.bookPic + '" data-bookauthor="' + Item.bookAuthor + '" data-bookname="' + Item.bookName + '" data-bookcontent="' + Item.bookContent + '" data-bookurl = "' + Item.bookUrl + '">';
...
function ItemBookInfo(bookpic,bookName,bookAuthor,bookContent) { //单个图书信息
	bookimg.src = bookpic;
