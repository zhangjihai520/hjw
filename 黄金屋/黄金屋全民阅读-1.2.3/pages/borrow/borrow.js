...
Page({
    ...
    wx.scanCode({
      success: (res) => {
        ...
        for (var i = 0; i < data.split("&").length; i++) {
          ...
          } else if (data.split("&")[i].split("=")[0] == "type") {
            qrType = data.split("&")[i].split("=")[1];
          } else if (data.split("&")[i].split("=")[0] == "bookShelfId") {
            bookShelfId = data.split("&")[i].split("=")[1];
          } else if (data.split("&")[i].split("=")[0] == "bookCode") {
            bookCode = data.split("&")[i].split("=")[1];
          } else if (data.split("&")[i].split("=")[0] == "take_shelfId") {
            take_shelfId = data.split("&")[i].split("=")[1];
        ...
        t.scanBorrow(qrType, take_shelfId, bookShelfId, bookCode, cellNo,shelfType);
      },
      fail: (res) => {
        console.log(res)
      }
   ...
   * 扫码借书请求方法
   */
  scanBorrow: function (qrType, take_shelfId, bookShelfId, bookCode, cellNo, shelfType) {
    var t = this;
    var url;
    var data = {

    };
    if (take_shelfId != "") {//扫码取书
      if (shelfType == 1) { //新柜子
