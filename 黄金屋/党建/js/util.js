var util = {
	...
	helper: {
		checkUpdate: function(){
			...
			var url = util.str.createAjaxURL(ao, ao.method_check_update);
			var data = {
				system: global_system[plus.os.name],
				...
				if(response.needUpdate){
					if(response.forceUpdate){
						mui.alert('发现新版本，请更新', global_stringObject.project, function() {
							plus.runtime.openURL(imgUrl + response.url);
						});
					}else{										
		...
		logOut: function(){
			...
			_localStorage.removeItem("User_Name");
			_localStorage.removeItem("User_PassWord");
