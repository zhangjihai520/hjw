		...
		switch(msgData.modeCode) {
			case "shelf_login":
				if(msgData.result == 1) {
					...
					document.getElementById('shelfName').innerHTML = '黄金屋:' + localStorage.getItem(global_ajaxObject.setting.method_shelf_Name);
				} else {
			...
			case "openDoor":
				...
				if(msgData.bookshelfId == public_getBookShelfId()) {
					public_openTake(msgData.cellNo);
				}
				break;
			case "lightOnOff":
				if(msgData.bookshelfId == public_getBookShelfId()) {
...
function public_getCardCode(toDo){
	var url = 'http://127.0.0.1:18001/readMajor';
	var data = {
		timeOut:60
	};
	var success = function(response) {
	...
	util.ajax.getJSONWaitingText(url, data, success);
}


function public_getBookCodeNum(toDo){
	var url = 'http://127.0.0.1:18001/readMajorBlock';
	var data = {
		timeOut:60
	};
	var success = function(response) {
